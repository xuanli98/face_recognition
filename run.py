...
choose = int(cf.get('db','db_facenet'))
...
def face_match(face1_output, face2_output):
    ...
    if (total_diff < FACE_MATCH_THRESHOLD):
        #CMP.append((input_image_file[0:len(input_image_file)-4],total_diff))
        #print(CMP)
...
def run_images(valid_output, validated_image_filename, graph, input_image_filename_list):
    ...
    for input_image_file in input_image_filename_list :
        ...
        matching = False
        diff = face_match(valid_output, test_output)
        if (diff):
            matching = True
            flag = 1
            name = input_image_file[0:len(input_image_file)-4]
            CMP.append((name,diff))
            #print(input_image_file[0:len(input_image_file)-4])
            #text_color = (0, 255, 0)
            #match_text = "MATCH"
    ...
    if flag ==1:
        #print(CMP)
        FACE=sorted(CMP, cmp=lambda x,y:cmp(x[1],y[1]))
        #print(FACE)
        FACE_final = FACE[0]
        print(FACE_final[0])
        sender_to_voice.send_data(FACE_final[0])
